...
{ 
  ...
  if(Design=="Pairwise" & pB==10 & s0_B==2 & rho==0.7 & SNR==target.snr) {pairwise.list=c(pairwise.list,folder)}
...
mtext(text=LETTERS[indicator],side=3,adj=0,cex=1)
                        ...
                        for(folder in list.settings) 
                       {
                            ...
                            if(p<=1000)         
                          {
                            ...
                            folder.name=sprintf("n=%s;p=%s;s0=%s;SNR=%s;Design=%s;rho=%s;pB=%s;s0_B=%s",n,p,s0,target.snr,design1,ifelse(design1=="Independence",0,0.7),ifelse(design1=="Independence",0,10),ifelse(design1=="Independence",0,2))
                            ...
                            folder.name=sprintf("n=%s;p=%s;s0=%s;SNR=%s;Design=%s;rho=%s;pB=%s;s0_B=%s",n,p,s0,target.snr,design2,ifelse(design2=="Independence",0,0.7),ifelse(design2=="Independence",0,10),ifelse(design2=="Independence",0,2))
                            setwd(sorted_address); setwd(folder.name)
